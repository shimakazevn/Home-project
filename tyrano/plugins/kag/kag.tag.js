    ...
    checkVital: function (tag) {
        ...
        for (var array_vital = master_tag.vital, i = 0; i < array_vital.length; i++)
            tag.pm[array_vital[i]]
                ? "" == tag.pm[array_vital[i]] && (err_str += "タグ「" + tag.name + "」にパラメーター「" + array_vital[i] + "」は必須です　\n")
...
tyrano.plugin.kag.tag.text = {
    ...
    showMessage: function (message_str, pm, isVertical) {
        ...
        if (("" != chara_name && "join" != pm.backlog) || ("" != chara_name && "true" == this.kag.stat.f_chara_ptext)) {
            this.kag.pushBackLog("<span class='backlog_text " + chara_name + "'>" + message_str + "</span>", "add")
        ...
        !(function (jtext) {
            ...
            "true" == that.kag.config.autoRecordLabel &&
                (1 == that.kag.stat.already_read
                    ? "default" != that.kag.config.alreadyReadTextColor && j_span.css("color", $.convertColor(that.kag.config.alreadyReadTextColor))
                    : "false" == that.kag.config.unReadTextSkip && (that.kag.stat.is_skip = !1))
            var ch_speed = 30
            "" != that.kag.stat.ch_speed
                ? (ch_speed = parseInt(that.kag.stat.ch_speed))
                : that.kag.config.chSpeed && (ch_speed = parseInt(that.kag.config.chSpeed))
            ...
            var append_span = j_span.children("span:last-child"),
                pchar = function (index) {
                        ...
                        (function (index) {
                            if ("" != that.kag.stat.font.effect) {
                                ...
                                append_span
                                    ...
                                    .css("animation", "t" + that.kag.stat.font.effect + " " + that.kag.stat.font.effect_speed + " ease 0s 1 normal forwards")
                    ...
                    if (index <= message_str.length) {
                        ...
                        1 == that.kag.stat.is_click_text || 1 == that.kag.stat.is_skip || 1 == that.kag.stat.is_nowait
                            ? pchar(++index)
                            : setTimeout(function () {
                                  pchar(++index)
                              }, ch_speed)
    ...
    start: function (pm) {
        ...
        "0" != pm.hmax
            ? $("." + this.kag.define.BASE_DIV_NAME).effect(
                  "shake",
                  {times: parseInt(pm.count), distance: parseInt(pm.hmax), direction: "left"},
                  parseInt(pm.time),
                  function () {
                      "true" == pm.wait && that.kag.ftag.nextOrder()
                  }
            ...
            : "0" != pm.vmax &&
              $("." + this.kag.define.BASE_DIV_NAME).effect(
                  "shake",
                  {times: parseInt(pm.count), distance: parseInt(pm.vmax), direction: "up"},
                  parseInt(pm.time),
                  function () {
                      "true" == pm.wait && that.kag.ftag.nextOrder()
                  }
        ...
        !(function () {
            ...
            j_button.click(function (event) {
                ...
                if ("" != _pm.role) {
                    ...
                    if (
                        !(
                            ("save" != _pm.role && "menu" != _pm.role && "quicksave" != _pm.role && "sleepgame" != _pm.role) ||
...
tyrano.plugin.kag.tag.clickable = {
    vital: ["width", "height"],
    pm: {width: "0", height: "0", x: "", y: "", border: "none", color: "", mouseopacity: "", opacity: "140", storage: null, target: null, name: ""},
    start: function (pm) {
...
tyrano.plugin.kag.tag.bg = {
    vital: ["storage"],
    pm: {storage: "", method: "crossfade", wait: "true", time: 3e3, cross: "false"},
    start: function (pm) {
        this.kag.ftag.hideNextImg()
        var that = this
...
tyrano.plugin.kag.tag.bg2 = {
    vital: ["storage"],
    pm: {name: "", storage: "", method: "crossfade", wait: "true", time: 3e3, width: "", height: "", left: "", top: "", cross: "false"},
    start: function (pm) {
        this.kag.ftag.hideNextImg()
        var that = this
        0 == pm.time && (pm.wait = "false")
